...
try {
    ...
    $sql = "SELECT id, second_name, name, third_name, birthdate, post, employment_status, work_id, login, password, code_word FROM workers WHERE post = 'Заправщик' AND employment_status = FALSE";
    $stmt = $db->query($sql);
    $emp = $stmt->fetchAll();
...
<?php
    ...
    <h2>Назначение заправки</h2>
    <form action="refill.php?idf=<?php echo urlencode($plane_id); ?>" method="post">
    ...
    <div class="form-row">
        ...
        <br>
            ...
            <button type="submit" class="button-sp">Назначить заправку</button>
...
form.addEventListener('submit', function(event) {
    ...
    if (hasUnfinishedWork) {
        alert('На этот самолет уже назначена заправка');
        event.preventDefault();
        return;
    }
